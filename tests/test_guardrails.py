...
def test_forbidden_path_blocks():
    ...
    with pytest.raises(GuardrailViolation) as e:
        g.check_action(a, predicted_class="schema_drift", confidence=0.9,
...
def test_blast_radius_files_blocks():
    ...
    with pytest.raises(GuardrailViolation):
        g.check_action(a, predicted_class="schema_drift", confidence=0.9,
                       diff="", files_changed=["a.sql", "b.sql", "c.sql"])
...
def test_destructive_op_blocks():
    ...
    diff = "--- a/x.sql\n+++ b/x.sql\n+DROP TABLE orders;\n"
    with pytest.raises(GuardrailViolation) as e:
        g.check_action(a, predicted_class="schema_drift", confidence=0.95,
                       diff=diff, files_changed=["x.sql"])
...
def test_low_confidence_blocks_non_pr_action():
    ...
    a = Action(kind="retry")
    with pytest.raises(GuardrailViolation) as e:
        g.check_action(a, predicted_class="upstream_5xx", confidence=0.4,
                       diff=None, files_changed=[])
...
def test_pr_kind_bypasses_whitelist():
    ...
    g = Guardrails()
    a = Action(kind="pr")
    g.check_action(a, predicted_class="auth_expiry", confidence=0.6,
                   diff="", files_changed=["x.sql"])
