...
def test_get_provider_with_failover():
    with patch.dict(os.environ, {
        "SHDPA_LLM_PROVIDER": "mock",
        "SHDPA_FALLBACK_PROVIDER": "mock"
    }):
...
def test_failover_provider_success():
    ...
    expected_resp = LLMResponse(
        text="success", prompt_tokens=1, completion_tokens=1,
        cost_usd=0.01, latency_ms=10, model="primary-model", provider="primary"
    )
...
def test_failover_provider_trigger():
    ...
    expected_resp = LLMResponse(
        text="fallback success", prompt_tokens=2, completion_tokens=2,
        cost_usd=0.02, latency_ms=20, model="fallback-model", provider="fallback"
    )
